				...
				numerical_type gpu_threads)
{
	...
	border_print();
	center_print("GPU Cross-Section Lookup Kernel, Naive (pure binary) search, Struct-of-Arrays",80);
	border_print();

	/*
	Calculate number of lookups each thread will have to perform.
	*/
	numerical_type num_lookups_per_thread = ceil( (float)num_lookups / ( (float) gpu_blocks * gpu_threads ) );
	...
	*/
	...
	/*
	Open file for writing out timing information, and start clock
	*/
	...
	*/
	...
	for (int i = 0; i < gpu_blocks * gpu_threads; i++){
		checksum += (double)( host_results_array[i] );
	}
	cout << checksum << endl;
	...
	*/
	numerical_type total_lookups = gpu_threads * gpu_blocks;
